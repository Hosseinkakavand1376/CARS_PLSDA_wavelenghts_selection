"""
Indian Pines Hyperspectral Dataset - CARS PLS-DA Wavelength Selection Experiment

This script demonstrates the application of CARS (Competitive Adaptive Reweighted Sampling)
with multi-class PLS-DA for wavelength selection on the Indian Pines hyperspectral dataset.

Dataset: Northwestern Indiana (145×145 pixels, 200 bands, 16 classes)
Objective: Select optimal wavelengths for crop/land cover classification
"""
...
print("="*80)
print("INDIAN PINES HYPERSPECTRAL DATASET - CARS PLS-DA EXPERIMENT")
print("="*80)
...
print(f"\nDataset Summary:")
print(f"  - Total samples: {X_indian.shape[0]:,}")
print(f"  - Number of bands: {X_indian.shape[1]}")
print(f"  - Number of classes: {len(np.unique(y_indian))}")
print(f"  - Classes: {sorted(np.unique(y_indian))}")

# Print class distribution
print(f"\nClass Distribution:")
unique, counts = np.unique(y_indian, return_counts=True)
for class_id, count in zip(unique, counts):
    class_name = INDIAN_PINES_CLASSES.get(class_id, f'Unknown_{class_id}')
    pct = 100 * count / len(y_indian)
    print(f"  Class {class_id:2d} ({class_name:30s}): {count:5,} samples ({pct:5.2f}%)")

# Note about class imbalance
print(f"\n[NOTE] Indian Pines has significant class imbalance:")
...
if NORMALIZE:
    ...
    X_normalized = np.zeros_like(X_indian, dtype=float)
    for i in range(X_indian.shape[0]):
        mean = np.mean(X_indian[i, :])
        std = np.std(X_indian[i, :])
        if std > 0:
            X_normalized[i, :] = (X_indian[i, :] - mean) / std
        else:
            X_normalized[i, :] = X_indian[i, :] - mean
    X_processed = X_normalized
    print("[OK] SNV normalization applied")
else:
    X_processed = X_indian
...
print(f"  {'-'*5} {'-'*8} {'-'*8}")
for class_id in sorted(np.unique(y_indian)):
...
output_dir = "experiments/indian_pines_results"
...
results_df = pd.DataFrame({
    'Class': clf.classes_,
    'Class_Name': [INDIAN_PINES_CLASSES.get(c, f'Class_{c}') for c in clf.classes_],
...
print("="*80)
print(f"\nKey Results:")
print(f"  ✓ Dataset: Indian Pines (10,249 samples, 200 bands, 16 classes)")
...
print(f"\n[NOTE] Due to class imbalance, weighted metrics may differ from macro metrics.")
print(f"        Small classes (e.g., Oats, Grass-pasture-mowed) may have lower scores.")
print("\n" + "="*80)
